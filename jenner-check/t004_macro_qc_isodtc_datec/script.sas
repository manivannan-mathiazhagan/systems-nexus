/* Caller for the qc_isodtc macro from this repo, in DATEC-only mode.        */
/* The macro is invoked inside a DATA step with three character date pieces */
/* (year, month abbreviation, day-of-month) plus a numeric time, and        */
/* assembles a SDTM-style ISO 8601 character DTC variable from them.         */
/* Year/month/day are derived through array lookups (no is8601da. informat   */
/* needed when the date arrives in piece form).                             */

/* Set the user-warning prefix that the macro emits when it finds invalid  */
/* date pieces (referenced inside the macro as &uwarn.).                   */
%let uwarn = %quote(USER_WARN:);

/* === macro under test (verbatim from SAS Programs/qc_isodtc.sas) === */
...
/* === small dataset of CRF date pieces (year/month/day) === */
data crf_dates;
    length DOB_YY $4 DOB_MO $3 DOB_DD $2;
    input DOB_YY $ DOB_MO $ DOB_DD $;
    DOB = catx(" ", DOB_YY, DOB_MO, DOB_DD);
    datalines;
2023 MAR 15
2023 APR 01
2022 DEC 31
...
/* call the macro inside a data step --- DATEC mode only */
data crf_dates_iso;
    set crf_dates;
    %qc_isodtc(DATEC=DOB, OUTDTC=BRTHDTC);
run;

proc print data=crf_dates_iso noobs;
    title "qc_isodtc (DATEC mode): year+month+day character pieces -> ISO 8601 DTC";
    var DOB BRTHDTC;
run;
