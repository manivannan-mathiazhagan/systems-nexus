...
%let sd_path=raw_v20230215\sdtm_v20230215;

%setenv(sponsor =CliniLabs,
        study   =Cybin CYB003-001 MDD MAD Ph1,
        dbver2  =&sd_path.);

proc datasets library=WORK memtype = data kill noprint;
quit;

%imp_xlsx(fname=Y:\mmathialagan\My SAS Files\Data\raw_issue_2_dm\get_raw_issues_Cybin.xlsx,Sheet=List);
...
proc sql noprint;
    create table chk as select distinct name into: chk1 from sashelp.vcolumn where libname eq "WORK" and name eq "SUBJECTID";
create table chk1 as select  memname,name  from sashelp.vcolumn where libname eq "WORK" and name eq "SUBJECTID";
quit;
...
%if "&chk1" eq "0" %then 
    %do;
        ...
        data ISS_CHK_1;
            length subjectid $200.;
            subjectid = "Site id information is not present in any of the raw datasets";
        run;
    %end;
