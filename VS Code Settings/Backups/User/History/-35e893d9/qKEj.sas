...
data _null_;
  set trtp_n;
  call symputx(cats("fmt_", strip(trtA)), cats(trtA, " (N=", n, ")"));
run;
...
proc sgplot data=max dattrmap=myattrmap;
	vbox ratio / category=param group=trtA attrid=TRTPC
...
data qcfig.&pgmqc;
	...
	format _all_;
	informat _all_;
	keep usubjid parcat1 paramcd param trtAn trtA adt atm aval ratio lbstnrhi;
run;
