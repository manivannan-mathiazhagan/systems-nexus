...
It reads all sheets and saves the content as a JSON dictionary in a format
compatible with SAS JSON libname.
...
def convert_xlsx_to_json(input_file: str, output_file: str) -> None:
    ...
    # Prevent pandas from interpreting "NA", "N/A", etc. as missing values
    excel_data = pd.read_excel(
        input_file,
        sheet_name=None,
        keep_default_na=False  # Preserve literal "NA", "N/A", etc.
    )

    # Convert each sheet's DataFrame to list of dictionaries
    excel_dict = {
        sheet_name: df.to_dict(orient="records")
        for sheet_name, df in excel_data.items()
    }

    # Wrap in a SAS-compatible structure
    wrapped_data = {
        "spreadsheets": [
            {"SheetName": name, "SheetData": data}
            for name, data in excel_dict.items()
        ]
    }

    # Dump to JSON, disallow NaN to enforce proper JSON (NaN → error, null allowed)
    with open(output_file, "w") as f:
        json.dump(wrapped_data, f, indent=2, allow_nan=False)
