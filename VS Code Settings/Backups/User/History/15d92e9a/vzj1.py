...
It reads all sheets and saves the content as a JSON dictionary.
...
def convert_xlsx_to_json(input_file: str, output_file: str) -> None:
    ...
    # Convert each sheet's DataFrame to list of row dictionaries
    excel_dict = {
        sheet_name: df.to_dict(orient="records")
        for sheet_name, df in excel_data.items()
    }

    # Wrap in 'spreadsheets' root key to be SAS-compatible
    wrapped_data = {
        "spreadsheets": [{"SheetName": name, "SheetData": data} for name, data in excel_dict.items()]
    }
