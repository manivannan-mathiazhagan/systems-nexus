...
************************************************************************************; 
* PROGRAM:    P:\Projects\Cook MyoSite\DIFI - 22-01\Biostats\DSMB\Tables\Macros\mt-14-3-1-ae-bio.sas     
* DATE:       31DEC2024
* PROGRAMMER: Laurie Drinkwater
*
* PURPOSE:      Macros for Tables 14.3.1.* Bio series. 
*
************************************************************************************;
* MODIFICATIONS: 
...
data all_2;
  	...
  	length result $12; 
   * pct = round((count/&col9)*100,.1);
   *result = put(count,3.) || ' (' || put((count/&col9)*100,5.1) || ')'; 
...
proc transpose data=all_3 out=all_42 prefix=cnt;
	where stvispdn = 1;
   	...
   	id trt;
   	var count;   
proc transpose data=all_3 out=all_49 prefix=pct;
	where stvispdn = 1;
   	by aebodsys aedecod;
   	id trt;
   	var pct;     
...
proc transpose data=all_3 out=uall_42 prefix=ucnt;
	where stvispdn = 2;
   	...
   	id trt;
   	var count;   
proc transpose data=all_3 out=uall_49 prefix=upct;
	where stvispdn = 2;
   	by aebodsys aedecod;
   	id trt;
   	var pct;     
...
data all_4;
   	merge all_41 all_42 all_49 
		  uall_41 uall_42 uall_49;
   	by aebodsys aedecod;
run; 
...
*===============================================================================;
data empty;
   	length formal $200 col: ucol: $11 ucol9 $1; 
    cat = 99;  formal = ''; 
...
%do i=1 %to 2;
	...
	%if &i=2 %then %do; ods rtf style=style1 file="&outdat\open session\&pgm..rtf"; %end;
	   ...
	   %if &i=1 %then %do; ("!S={borderbottomwidth=1}Blinded Treatment Period" col1 col2 col3) %end;  space
	    %if &i=1 %then %do; ("!S={borderbottomwidth=1}Unblinded Treatment Period" ucol1 ucol2 ucol3) %end;   
      %if &i=2 %then %do;  col9 space    ucol9; %end; 
	  ;
