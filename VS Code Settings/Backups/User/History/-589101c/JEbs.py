...
# Auto-install required packages for GUI TLF Packager (user install only, no admin rights)
import sys
import subprocess
import site
...
class TLFApp(QtWidgets.QWidget):
    ...
    def __init__(self):
        ...
        # Professional tip message, not yellow
        ...
        self.table.setStyleSheet("""
            ...
            QTableWidget::item { border-radius: 8px; padding: 2px;}
            QTableWidget::item:selected { background: #356AC3; color: #fff; }
            QTableWidget::item:focus { border: 1px solid #356AC3; }
        """)
        ...
        header_view.setSectionResizeMode(5, QtWidgets.QHeaderView.Stretch)
        ...
        for i in range(len(self.labels)):
            self.table.horizontalHeaderItem(i).setToolTip("Click to sort by this column")
        ...
        self.select_btn.setStyleSheet(btn_base_style + """
            QPushButton { background-color: #33b9b2; color: #fff; }
            QPushButton:hover { background-color: #4eddd7; }
        """)
        btn_layout.addWidget(self.select_btn)

        self.deselect_btn = QtWidgets.QPushButton("Deselect All")
        self.deselect_btn.setStyleSheet(btn_base_style + """
            QPushButton { background-color: #33b9b2; color: #fff; }
            QPushButton:hover { background-color: #4eddd7; }
        """)
        ...
        self.select_btn.setToolTip("Check all files for inclusion (filtered/visible rows only).")
        self.deselect_btn.setToolTip("Uncheck all files (filtered/visible rows only).")
        self.export_btn.setToolTip("Export table (and bookmark info) to Excel file.")
        self.up_btn.setToolTip("Move selected file one row up.")
        self.down_btn.setToolTip("Move selected file one row down.")
        self.convert_btn.setToolTip("Convert all checked RTF/DOCX files to PDF using Word.")
        self.pack_btn.setToolTip("Generate combined PDF and add clickable Table of Contents.")
    ...
    def refresh_table(self):
        ...
        for ridx, i in enumerate(indices):
            ...
            for col, val in enumerate(disp_row):
                ...
                # Include column: just normal, left-aligned, no special color
                if col == 0:
                    item.setFont(QtGui.QFont("Times New Roman", 10))
                    item.setTextAlignment(QtCore.Qt.AlignCenter)
                    item.setBackground(base_bg)
                # Type column: colored based on value
                elif col == 1:
                    item.setFont(QtGui.QFont("Times New Roman", 10))
                    item.setTextAlignment(QtCore.Qt.AlignCenter)
                    ftype = rowdata[1].upper()
                    if ftype == "RTF":
                        item.setBackground(QtGui.QColor("#D6E6FA"))    # Blue
                    elif ftype == "PDF":
                        item.setBackground(QtGui.QColor("#FFE0BB"))   # Orange
                    elif ftype == "DOCX":
                        item.setBackground(QtGui.QColor("#C9F6C9"))   # Green
                    else:
                        item.setBackground(base_bg)
                # Bookmark column: no special color, left-aligned
                elif col == 6:
                    item.setFont(QtGui.QFont("Times New Roman", 10))
                    item.setTextAlignment(QtCore.Qt.AlignLeft | QtCore.Qt.AlignVCenter)
                    item.setBackground(base_bg)
                # Other columns: alternate row color, left
                else:
                    item.setFont(QtGui.QFont("Times New Roman", 10))
                    item.setTextAlignment(QtCore.Qt.AlignLeft | QtCore.Qt.AlignVCenter)
                    item.setBackground(base_bg)
    ...
    def set_button_states(self, browse=True, others=False, convert=False, pack=False):
        ...
        for btn in [self.select_btn, self.deselect_btn, self.export_btn, self.up_btn, self.down_btn]:
    ...
    def refresh_folder(self, full_rescan=False):
        ...
        # Add new files in the order discovered
        ...
        # --- Initial sort only, never sort again unless user clicks header ---
    ...
    def select_all(self):
        indices = self.filtered_indices if self.filtered_indices else list(range(len(self.table_data)))
        for idx in indices:
            self.table_data[idx][0] = True
        self.refresh_table()

    def deselect_all(self):
        indices = self.filtered_indices if self.filtered_indices else list(range(len(self.table_data)))
        for idx in indices:
            self.table_data[idx][0] = False
        self.refresh_table()
        ...
        def worker():
            try:
                for idx, row in enumerate(files_to_convert):
                    ...
                    self.thread_safe_log(f"[{idx+1}/{total}] Processing: {fname} ({ftype} to PDF conversion started)")
                    file_path = os.path.join(self.out_folder, fname)
                    pdf_path = convert_to_pdf_with_word(file_path, self.out_folder)
                    self.thread_safe_log(f"[Completed] {ftype} to PDF conversion for {fname}")
                self.thread_safe_log("All selected RTF/DOCX files converted to PDF.")
                self.thread_safe_message("done", "Selected RTF/DOCX files have been converted to PDF.\n(See log for details.)")
            except Exception as e:
                import traceback
    ...
    def generate_pdf(self):
        if not self.table_data or not self.out_folder:
            self.thread_safe_message("error", "No data or folder selected.")
            return
        output_pdf = self.outpdf_var.text().strip()
        ...
        self.thread_safe_log(f"Total {rtf_count} RTF, {pdf_count} PDF, {docx_count} DOCX selected for packing.")
        ...
        def worker():
            try:
                ...
                for idx, row in enumerate(files_to_pack):
                    ...
                    self.thread_safe_log(f"[{idx+1}/{total}] Processing: {fname} ({ftype})")
                    file_path = os.path.join(self.out_folder, fname)
                    if ftype in ['RTF', 'DOCX']:
                        self.thread_safe_log(f"[{idx+1}/{total}] {ftype} to PDF conversion started.")
                        pdf_path = convert_to_pdf_with_word(file_path, self.out_folder)
                        temp_pdfs.append(pdf_path)
                        self.thread_safe_log(f"[{idx+1}/{total}] {ftype} to PDF conversion completed.")
                    else:
                        pdf_path = file_path
                ...
                self.thread_safe_log(f"[Compiling] Generating final merged PDF with TOC...")
                merged.set_toc(toc)
                ...
                if os.path.exists(temp_bookmarked_pdf): os.remove(temp_bookmarked_pdf)
                self.thread_safe_log(f"[Done] PDF created: {output_path}")
                self.thread_safe_message("done", f"PDF Generated!\nOutput: {output_path}")
            except Exception as e:
                import traceback
                tb = traceback.format_exc()
                self.thread_safe_log(f"ERROR: {e}\n{tb}")
