...
*** Updated on:         $Date:: 2025-08-12 22:45:07 +0530 (Tue, 12 Aug 2025)   $:                   ***;
*** Version No:         $Rev:: 211    $:                                                            ***;
***                                                                                                 ***;
***-------------------------------------------------------------------------------------------------***;
%setenv(sponsor=Covis Pharma, study=Feraheme Imaging Supplemental, dbver2
    =raw_v20250729\sdtm_v20250829);

%include "&proot\SDTM\SDTM_Macros.sas";

%macro yes_pt(yesdsn);

    %imp_gdocjson( sheet=&yesdsn,
        gdoc_key=1EAf_6-H0aNTMCktK5P6KHy1pX-kKbWbjz794FTNvNlI, outlib=sdtmspec,
        force=1, add_rownums=1);
    %sdtm_create(speclib=sdtmspec, specds=&yesdsn, outlib=SDTM,
        pgmpath=&proot\Sdtm, source_id_vars=usubjid,studyid=);
%mend;
