        ...
        try:
            ...
            def build_hf_section(lines):
                ...
                for i in sorted(lines.keys()):
                    l, c, r = lines[i]
                    parts = []
                    if l: parts.append(f"\\ql {l}")
                    if c: parts.append(f"\\qc {c}")
                    if r: parts.append(f"\\qr {r}")
                    result.append("\\par ".join(parts))
                return "\\par ".join(result)

            rtf += f"\\header \\pard {build_hf_section(self.header_footer_data['Header'])} \\par\n"
            rtf += f"\\footer \\pard {build_hf_section(self.header_footer_data['Footer'])} \\par\n"

            # Titles
            for i in range(1, 4):
                title = getattr(self, f"title{i}_edit").toPlainText().strip()
                if title:
                    rtf += f"\\qc\\b {title} \\b0\\par\n"

            rtf += "\\trowd\\trgaph108\\trleft-108\n"
            rtf += "\\cellx3000\\cellx6000\\cellx9000\n"

            for r in range(self.grid.rowCount()):
                text = self.grid.item(r, 0).text()
                fmt = self.grid.cellWidget(r, 1).currentText()
                val = self.grid.item(r, 2).text()
                rtf += f"\\intbl {text}\\cell {fmt}\\cell {val}\\cell\\row\n"
