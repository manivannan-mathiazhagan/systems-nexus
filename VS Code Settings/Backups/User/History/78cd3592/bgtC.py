...
from PyQt5.QtWidgets import (
    ...
    QTableWidget, QTableWidgetItem, QGroupBox, QTextEdit, QScrollArea, QFormLayout
)
...
class TableShellTool(QWidget):
    ...
    def setup_ui(self):
        ...
        self.table_dropdown = QPushButton("Select Table ID")
        self.table_dropdown_menu = QScrollArea()
        self.table_dropdown_menu.setWidgetResizable(True)
        self.table_dropdown_group = QGroupBox()
        self.table_dropdown_layout = QVBoxLayout()
        self.table_dropdown_group.setLayout(self.table_dropdown_layout)
        self.table_dropdown_menu.setWidget(self.table_dropdown_group)
        self.table_dropdown.setMenu(None)
        table_layout.addWidget(QLabel("Table ID:"))
    ...
    def load_excel(self):
        ...
        # Update table ID dropdown
        self.table_dropdown_layout.setParent(None)
        self.table_dropdown_layout = QVBoxLayout()
        for r in self.excel_data["Table Runs"]:
            tnum = str(r[1].value).strip()
            btn = QPushButton(tnum)
            btn.clicked.connect(lambda _, tid=tnum: self.load_table_data(tid))
            self.table_dropdown_layout.addWidget(btn)
        self.table_dropdown_group.setLayout(self.table_dropdown_layout)
    ...
    def load_table_data(self, table_id):
        ...
        titles = [f"Table {table_id}"]
        if table_row:
            titles += [v.value or "" for v in table_row[9:12]]
            titles = [t.replace("&ParameterLabel", param_label) for t in titles]
        if popid in self.excel_data["Populations"]:
            titles.append(self.excel_data["Populations"][popid])
    ...
    def export_rtf(self):
        ...
        try:
            with open(path, "w") as f:
                ...
                f.write("\\fs16\n")
                for section in [
                    self.header_text, self.title_text, self.content_table, self.footnotes_text, self.footer_text
                ]:
    ...
    def export_pdf(self):
        ...
        try:
            ...
            word.Quit()
            os.remove(rtf_temp)
            QMessageBox.information(self, "PDF Saved", f"Saved: {rtf_path}")
        except Exception as e:
            QMessageBox.critical(self, "Error", str(e))
