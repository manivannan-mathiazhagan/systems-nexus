...
class TableShellTool(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("🧾 Table Shell Exporter v4.3")
    ...
    def load_table_content(self, tid):
        ...
        for i in range(3):
            ...
            self.grid.setItem(i, 0, QTableWidgetItem(f"Row {i+1}"))
            cb = QComboBox()
            cb.addItems(["n","n(%)","Mean(SD)","Min","Max","NPCT"])
            self.grid.setCellWidget(i, 1, cb)
            self.grid.setItem(i, 2, QTableWidgetItem("xx"))
    ...
    def export_rtf(self):
        ...
        header_txt = "".join(
            rf"\pard\ql\f0\fs16 {l}\tab\qc\f0\fs16 {c}\tab\qr\f0\fs16 {r}\par"
