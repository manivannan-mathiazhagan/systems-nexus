...
class TableShellTool(QWidget):
    ...
    def load_table_data(self, table_id):
        ...
        footnotes = [table_row[12].value, table_row[13].value, table_row[14].value] if len(table_row) >= 15 else []

        # Substitution
        title_parts = [t.replace("&ParameterLabel", param_label) if t else "" for t in title_parts]
        footnotes = [f.replace("&ParameterLabel", param_label) if f else "" for f in footnotes]
        ...
        # Titles
        rows.append(f"{'Table ' + str(tnum):^120}")
        if title_parts[0]: rows.append(f"{title_parts[0]:^120}")
        if title_parts[1]: rows.append(f"{title_parts[1]:^120}")
        if popname:        rows.append(f"{popname:^120}")
