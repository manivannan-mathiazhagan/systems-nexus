        ...
        if header_sheet:
            ...
            for row in ws.iter_rows(min_row=2, values_only=True):
                ...
                if section in ["Header", "Footer"] and isinstance(line, int):
                    ...
                    for i in range(3):
                        if row[i + 2]:
                            self.header_footer_data[section][line][i] = str(row[i + 2]).strip()
        ...
        try:
            ...
            def build_hf(lines):
                ...
                for i in sorted(lines.keys()):
                    l, c, r = lines[i]
                    seg = ""
                    if l: seg += rf"\pard\ql {l.strip()} "
                    if c: seg += rf"\pard\qc {c.strip()} "
                    if r: seg += rf"\pard\qr {r.strip()} "
                    output.append(seg + r"\par")
                return "".join(output)
