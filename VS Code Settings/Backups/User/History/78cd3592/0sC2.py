    ...
    def export_rtf(self):
        ...
        try:
            rtf = [
                r"{\rtf1\ansi\deff0",
                r"{\fonttbl{\f0 Arial;}}",
                r"\paperw16840\paperh11900\landscape",  # Landscape A4
                r"\margl1440\margr1440\margt1440\margb1440",  # 1 inch margins
            ]

            def build_hf_section(lines, alignment):
                out = []
                for i in sorted(lines.keys()):
                    l, c, r = lines[i]
                    if l: out.append(r"\ql " + l)
                    if c: out.append(r"\qc " + c)
                    if r: out.append(r"\qr " + r)
                return r"\par ".join(out)

            # Header & footer declarations
            rtf.append(r"{\header \pard " + build_hf_section(self.header_footer_data['Header'], 'H') + r"\par}")
            rtf.append(r"{\footer \pard " + build_hf_section(self.header_footer_data['Footer'], 'F') + r"\par}")

            # Title lines (centered, bold first line)
            for i in range(1, 4):
                title = getattr(self, f"title{i}_edit").toPlainText().strip()
                if title:
                    if i == 1:
                        rtf.append(r"\pard\qc\b " + title + r" \b0\par")
                    else:
                        rtf.append(r"\pard\qc " + title + r"\par")
