...
class TableShellTool(QWidget):
    ...
    def render_header_footer_preview(self):
        def build_text(data):
            ...
            for i in sorted(data.keys()):
                ...
                center = center.strip() if center else ""
    ...
    def export_rtf(self):
        path, _ = QFileDialog.getSaveFileName(
            self, "Save RTF File", f"{self.current_table_id}.rtf", "RTF Files (*.rtf)"
        )
        if not path:
            return
        ...
        try:
            rtf = [
                ...
                r"\fs16",  # 8pt font (fs = 2 x point size)
                r"\paperw16840\paperh11900\landscape",  # A4 landscape
            ...
            def build_hf_section(lines):
                ...
                for i in sorted(lines.keys()):
                    ...
                    line_parts = []
                    if l:
                        line_parts.append(r"\pard\ql\f0 " + l + r"\par")
                    if c:
                        line_parts.append(r"\pard\qc\f0 " + c + r"\par")
                    if r:
                        line_parts.append(r"\pard\qr\f0 " + r + r"\par")
                    out.extend(line_parts)
                return "\n".join(out)

            # Header & Footer
            rtf.append(r"{\header " + build_hf_section(self.header_footer_data["Header"]) + r"}")
            rtf.append(r"{\footer " + build_hf_section(self.header_footer_data["Footer"]) + r"}")

            # Title lines (no bold, all left-aligned or centered if needed)
            for i in range(1, 4):
                title = getattr(self, f"title{i}_edit").toPlainText().strip()
                if title:
                    rtf.append(r"\pard\qc\f0 " + title + r"\par")

            # Table header row setup
            ...
            for r in range(self.grid.rowCount()):
                ...
                rtf.append(rf"\intbl\f0 {text}\cell {fmt}\cell {val}\cell\row")
