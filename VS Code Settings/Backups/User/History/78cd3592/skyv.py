...
class TableShellTool(QWidget):
    ...
    def load_table_content(self, tid):
        ...
        def sub_title(text):
            if not text:
                return ""
            return re.sub(r"&parameterlabel", param_label, text, flags=re.IGNORECASE)

        raw_titles = self.title_2_map.get(pgm, ["", "", ""])
        raw_title1 = f"Table {tid}"
        raw_title2 = raw_titles[0]
    ...
    def render_header_footer_preview(self):
        def join_line(items):
            parts = []
            if items[0]: parts.append(f"{items[0]:<30}")
            if items[1]: parts.append(f"{items[1]:^50}")
            if items[2]: parts.append(f"{items[2]:>30}")
            return " | ".join(parts)

        header_lines = [join_line(self.header_footer_data["Header"][k]) 
                        for k in sorted(self.header_footer_data["Header"])]
        footer_lines = [join_line(self.header_footer_data["Footer"][k]) 
                        for k in sorted(self.header_footer_data["Footer"])]

        self.header_preview.setPlainText("\n".join(header_lines))
        self.footer_preview.setPlainText("\n".join(footer_lines))
