...
class TableShellTool(QWidget):
    ...
    def load_excel(self):
        ...
        for row in runs.iter_rows(min_row=2, values_only=True):
            ...
            if tid and pgm:
                self.excel_data[str(tid).strip()] = {
                    "pgm": str(pgm).strip(),
                    "popid": str(popid or "").strip(),
                    "paramids": str(paramids or "").strip()
                }
                self.table_dropdown.addItem(str(tid).strip())
        ...
        for row in params.iter_rows(min_row=2, values_only=True):
            pid, label = str(row[0]).strip(), str(row[2]).strip()
    ...
    def load_table_content(self, tid):
        ...
        pgm, popid, paramids = data.get("pgm", ""), data.get("popid", ""), data.get("paramids", "")
        param_label = self.param_labels.get(paramids, "") if paramids else ""

        def sub_title(text):
            return text.replace("&ParameterLabel", param_label) if text else ""

        self.title1_edit.setText(sub_title(f"Table {tid}"))
        self.title2_edit.setText(sub_title(self.title_2_map.get(pgm, "")))
        self.title3_edit.setText(sub_title(self.title_3_map.get(popid, "")))
        ...
        for r in range(3):
            ...
            self.grid.setItem(r, 2, QTableWidgetItem("xx"))    def render_header_footer_preview(self):
        def build(data):
            lines = []
            for i in sorted(data.keys()):
                l, c, r = data[i]
                line = f"{l:<30}{c:^40}{r:>30}".rstrip()
                lines.append(line)
            return "\n".join(lines)
        self.header_preview.setText(build(self.header_footer_data["Header"]))
        self.footer_preview.setText(build(self.header_footer_data["Footer"]))
    ...
    def add_row(self):
        ...
        self.grid.setCellWidget(r, 1, combo)
        ...
        try:
            ...
            def build_hf(lines):
                ...
                for i in sorted(lines.keys()):
                    l, c, r = lines[i]
                    row = r"\trowd\trgaph108"
                    row += r"\cellx3000\cellx9000\cellx15000"
                    row += rf"\intbl {l}\cell {c}\cell {r}\cell\row"
                    rows.append(row)
                return "\n".join(rows)
            ...
            rtf.append(r"\trowd\trgaph108")
            rtf.append(r"\cellx5000\cellx10000\cellx15000")
            for r in range(self.grid.rowCount()):
                t = self.grid.item(r, 0).text() if self.grid.item(r, 0) else ""
                f = self.grid.cellWidget(r, 1).currentText()
                v = self.grid.item(r, 2).text() if self.grid.item(r, 2) else ""
                rtf.append(rf"\intbl {t}\cell {f}\cell {v}\cell\row")
            rtf.append("}")
