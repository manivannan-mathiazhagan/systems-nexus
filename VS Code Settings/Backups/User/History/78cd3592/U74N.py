    ...
    def export_rtf(self):
        ...
        try:
            rtf = [
                ...
                r"\fs16",  # 8pt (half-points in RTF, so 8pt = 16)
            ...
            def build_hf_section(lines):
                ...
                for i in sorted(lines.keys()):
                    ...
                    segs = []
                    if l.strip():
                        segs.append(rf"\pard\ql {l.strip()}")
                    if c.strip():
                        segs.append(rf"\pard\qc {c.strip()}")
                    if r.strip():
                        segs.append(rf"\pard\qr {r.strip()}")
                    if segs:
                        result.append("".join(segs) + r"\par")
                return "".join(result)
            ...
            # Titles (no bolding, same font, font size 8)
            for i in range(1, 4):
