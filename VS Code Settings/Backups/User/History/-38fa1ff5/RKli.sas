...
%***currently, all variables are long character to capture everything (future enhancement to have user specify types/input on variables (we will not try to guess)***;
  ...
  filename tmppipe pipe "c:\bsp_apps\7za\7za x -y -o&usertemp\&sysjobid\ddtemp ""&fname"" ";
  data _wuz_result;
    infile tmppipe length=l;
    input line $varying500. l;
  run;
  ...
  data _styles_xf1; set styles.xf1; idx=_n_-1; run;
  proc sql;
    create table _cellstyle as
    select xf1.idx, xf1.numFmtId, xf1.xfId, cellstyle.name1, cellstyle.builtinId, numFmt.formatCode
    from _styles_xf1 as xf1 left join styles.cellstyle on xf1.xfId=cellstyle.xfId
                    left join styles.numFmt on xf1.numFmtId=numFmt.numFmtId
    order by xf1.idx
    ;
  quit;

  filename  sharedSt "!TEMP\&sysjobid\ddtemp\xl\sharedStrings.xml"; ** encoding="UTF-8";
  ...
  data _sst; set sharedst.si; if tnum=. then tnum=_n_; tnum=tnum-1; run;
...
*/
  ...
  ** using Relationships table is more reliable for sheet ID loading **;
  ...
  proc sql noprint;
    create table _workbook as
    select s.*, r.Target, r.Type
    from workbk.sheet s left join workbkr.relationship r
    on s.r_Id = r.Id;
  quit;
  ...
  data &outlib.._wkbk;
    ...
    *rId=sheetId;
    strcode="filename  sheet ""!TEMP\&sysjobid\ddtemp\xl\"||compress(target)||""";"; call execute(strcode);
    strcode="libname   sheet xml xmlmap=SXLEMAP access=READONLY;"; call execute(strcode);
    strcode="%nrstr(%%imp_xlsx_inc)("||compress(sheetId)||","||compress(name)||",hdrrow=&hdrrow,datarow=&datarow);"; call execute(strcode);
    strcode="proc append base=_imp_logall data=_imp_log; run;"; call execute(strcode);
    strcode="data &outlib.."||compress(name)||";"; call execute(strcode);
    strcode="  set _sheet"||compress(sheetId)||"; **drop _label_; run;"; call execute(strcode);
    option nosource;
  run;
  option source;
  %let __drops=&__drops _wkbk _workbook _wuz_result _sst _c0 _c _cval _cval1 _tdata _imp_log _imp_logall _tdatac _tdatan _sheetc _sheetn _attrib _attrc _attrn _sheet:;
