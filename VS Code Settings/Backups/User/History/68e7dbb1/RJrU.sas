...
%do i=1 %to 2;
	...
	%if &i=2 %then %do; ods rtf style=style1 file="&outdat\open session\&pgm..rtf"; %end;
   	...
   	proc report data=final split='~' nowd missing spanrows
		...
		%if &i=1 %then %do; ("!S={borderbottomwidth=1}Blinded Treatment Period" col1 col2 col3) space 
					("!S={borderbottomwidth=1}Unblinded Treatment Period" ucol1 ucol2 ucol3) ; %end; 
