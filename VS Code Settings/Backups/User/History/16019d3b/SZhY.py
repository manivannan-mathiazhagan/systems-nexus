...
# Ensures the Python user-specific site-packages directory is present in sys.path
def ensure_user_site():
    user_site = site.getusersitepackages()
    if user_site not in sys.path:
        sys.path.insert(0, user_site)

# Installs a package into user site-packages if missing, using optional import name check
def install_if_missing(package_name, import_name=None):
    try:
        __import__(import_name or package_name)
    except ImportError:
        print(f"[Installing] {package_name} (this will go into your user directory)...")
...
# Prints a banner-style separator with the given message
def banner(msg):
    print("\n" + "=" * 100)
    print(f" {msg}")
    print("=" * 100)

# Prints the main tool header with author and purpose
def print_header():
    print("=" * 100)
...
# Extracts RTF header content groups enclosed in braces
...
# Converts an RTF group string into plain text, stripping RTF control codes
...
# Reads an RTF file and extracts header lines containing table-like patterns
...
# Checks if a line is a valid candidate for a title (filters out notes, all-caps, etc.)
...
# Removes any trailing 'Note:' text from a line
def filter_note_from_line(text):
    return re.split(r'\bNote\s*:', text, flags=re.IGNORECASE)[0].strip()

# Extracts titles (Title1–Title3) and bookmark from an RTF file
...
# Extracts titles (Title1–Title3) and bookmark from the first page of a PDF file
...
# Extracts titles (Title1–Title3) and bookmark from a DOCX file (header and body search)
...
# Creates a tuple for sorting bookmarks based on type and numbering
...
# Writes extracted titles and bookmarks to a formatted Excel file for review
...
# Pauses execution for user Excel review (GUI prompt or console input)
...
# Reads reviewed titles and bookmarks back from Excel into a list
...
# Converts RTF/DOCX to PDF using Microsoft Word automation
...
# Merges PDFs into one file with bookmarks; optionally deletes converted files
...
# Wraps a TOC entry title into multiple lines to fit within max width
...
def add_toc_and_links(bookmarked_pdf, final_pdf, font_size=8):
    ...
    # Generates PDF pages for the Table of Contents and collects clickable link areas
    ...
    # Adds clickable links from TOC entries to their corresponding PDF pages
    ...
    # Adjusts bookmark page numbers by a given offset (e.g., when TOC pages are added)
    ...
    # Inserts adjusted bookmarks into the final PDF
...
# Main process flow: detect inputs, extract titles, Excel review, merge PDFs, add TOC
