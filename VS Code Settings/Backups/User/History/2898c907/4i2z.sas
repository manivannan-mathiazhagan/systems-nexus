...
************************************************************************************;
* PROGRAM:    P:\Projects\Cook MyoSite\DIFI - 22-01\Biostats\DSMB\_Restricted\Tables\t-14-1-3-2-mh.sas   
* DATE:       04NOV2024
...
/*	if vagnum le 0 then vagnum = .;*/
/*	else if vagnum > 2 then vagnum = 3; */
/*	else vagnum = vagnum;*/
...
%macro univ(order=,var=);                                                       
	...
	run;  
  	...
  	data temp;
        ...
        value = 3; if stderr ne . then result = '  '||strip(put(stderr,8.3));output;
        value = 4; if . < lclm < 0 then lclm = 0;
		      if lclm ne . and uclm ne . then result = '  ('||strip(put(lclm,7.2))||', '||strip(put(uclm,7.2))|| ')';
			  else if lclm eq . and uclm ne . then result = '  (, '||strip(put(uclm,7.2))||')';
			  else if lclm ne . and uclm eq . then result = '  ('||strip(put(lclm,7.2))||', '||')';
			  else if lclm eq . and uclm eq . then result = 'NA';output;

        value = 5; result = '  ' || compress(put(median,7.2)); output;
...
%dummy(order=%str(6),value=%str(1,2,3,4,5,6))   
