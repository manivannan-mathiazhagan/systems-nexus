...
#                  After user approval, combines all into a bookmarked PDF with TOC.
...
#
# Example Usage
#   python TLF_Packager.py "input_folder_path" "output_file_name.pdf" Y
#
# Notes
...
def main(folder, output_pdf_name="TLFs.pdf", delete_intermediate_pdfs=False):
    print_header()
...
if __name__ == "__main__":
    if len(sys.argv) < 3 or len(sys.argv) > 4:
        print("Usage: python TLF_Packager.py <folder_path> <output_pdf_name> [delete_converted_pdfs: Y/N]")
        sys.exit(1)
    path = sys.argv[1]
    ...
    if not os.path.isdir(path):
        ...
        sys.exit(1)
    main(path, output_pdf_name, delete_intermediate_pdfs)
