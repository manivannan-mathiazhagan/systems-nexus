...
def launch_notepadpp(filepath, line_number=None):
    try:
        if line_number:
            subprocess.Popen(["notepad++", f"-n{line_number}", filepath])
        else:
...
class TextSearchTool(QWidget):
    ...
    def check_compare_results(self):
        ...
        passed_kw = "compare procedure passed"
        failed_kw = "compare procedure failed"
        ...
        for root, _, files in os.walk(folder):
            for file in files:
                ...
                result = None
                if re.search(r"compare.*passed", content, re.IGNORECASE):
                    result = "Passed"
                    passed += 1
                elif re.search(r"compare.*failed", content, re.IGNORECASE):
                    result = "Failed"
                    failed += 1
                else:
                    continue
