...
#
# Purpose        : GUI Tool to search text across SAS, LOG, and TXT files in a folder,
...
#   - Folder Selection           : User selects the folder containing SAS, LOG, TXT, PDF, and LST files.
#   - Search String              : Case-insensitive text search within selected file types.
#   - Comparison File Type       : Select PDF, LST, or TXT to check PROC COMPARE results.
#   - Comparison Status Filter   : Filter only Passed, Failed, or All results.
#
# Key Features:
#   - Search for any string across .sas, .txt, and .log files with line preview and open in Notepad++.
...
#   - Supports professional GUI layout with vibrant UI and intuitive navigation.
#
# Example Usage:
#   - Run the script: python Veristat_TextSearch_CompareChecker.py
#   - Use the GUI to choose between "Text Search" and "Check PROC COMPARE Results"
#
# Notes:
#   - Requires Python 3, PyQt5, and PyMuPDF (fitz).
#   - Requires Notepad++ to be installed (path configurable).
...
def launch_notepadpp(filepath, line_number=None):
    npp_path = r"C:\Program Files\Notepad++\notepad++.exe"  # Customize path if needed
    try:
        if line_number:
        ...
        for root, _, files in os.walk(folder):
            for file in files:
                if ext != "All" and not file.lower().endswith(ext):
                    continue
                if not any(file.lower().endswith(x) for x in [".sas", ".txt", ".log"]):
                    continue
                total += 1
