...
************************************************************************************;
* PROGRAM:    P:\Projects\Cook MyoSite\DIFI - 22-01\Biostats\DSMB\Tables\t-14-3-4-lb-chem.sas     
* DATE:       06JAN2025
* PROGRAMMER: Laurie Drinkwater  
* PURPOSE:    Clinical Laboratory Results and Change from Baseline - Chemistry
*             (Safety Set)
*
************************************************************************************;
* MODIFICATIONS:   
...
*===============================================================================;    
data all_1;
	set ads.adlb (where=(saffl='Y' and trtan ne . and parcat1 = 'CHEMISTRY' and anl01fl='Y' and stvispd^=''));    
	...
	%if &i=2 %then %do; ods rtf style=style1 file="&outdat\open session\&pgm..rtf"; %end;
   	...
   	proc report data=final split='~' nowd missing spanrows
		style(report)=[rules=groups cellspacing=0];      
       	column page paramn param stvispd avisitn order formal space stat %if &i=1 %then %do; col1 col2 %end; %if &i=2 %then %do; col3 %end; ;
		...
		define formal   / style=[cellwidth=18% just=left] "Visit" flow;  
	  	...
	  	%end; %if &i=2 %then %do;
  		define col3     / style=[cellwidth=13.5% just=left] " Overall~ (N=&col3)";  %end; 
