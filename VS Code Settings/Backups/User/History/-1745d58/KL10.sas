...
************************************************************************************; 
* PROGRAM:    P:\Projects\Cook MyoSite\DIFI - 22-01\Biostats\DSMB\Tables\Macros\mt-14-3-1-ae_new-spcom.sas     
* DATE:       31DEC2024
...
data ads;
  set ads.adsl;
  if trt01a ne '' then trt01an=99;
 run;

proc freq data =ads;
	tables trt01an/out=denoall;
	where trt01an ne .;
run;

data denoall;
  set denoall;
   	if trt01an = 99 then call symput("col99",compress(put(count,3.))); 
run;
...
data all_2;
	...
	IF STVISPDN=1 THEN DO;
   	...
   	if trt = 9 then do; pct = round((count/&col9)*100,.1); result = put(count,3.) || ' (' || put((count/&col9)*100,5.1) || ')'; end;  else  
    if trt = 99 then do; pct = round((count/&col99)*100,.1); result = put(count,3.) || ' (' || put((count/&col99)*100,5.1) || ')'; end;
	END;
	IF STVISPDN=2 THEN DO;
   	...
   	if trt = 9 then do; pct = round((count/&col9)*100,.1); result = put(count,3.) || ' (' || put((count/&col92)*100,5.1) || ')'; end;  else  
	if trt = 99 then do; pct = round((count/&col99)*100,.1); result = put(count,3.) || ' (' || put((count/&col99)*100,5.1) || ')'; end; 
	END;
run; 
	...
	%if &i=2 %then %do; ods rtf style=style1 file="&outdat\open session\&pgm..rtf"; %end;
   	...
   	proc report data=final split='~' nowd missing spanrows
		style(report)=[rules=groups cellspacing=0];  
       	column n1 aebodsys n2 aedecod formal space stat
		%if &i=1 %then %do; ("!S={borderbottomwidth=1}Blinded Treatment Period" col1 col2 col3 ) col9 %end; 
		%if &i=2 %then %do;col99 ucol9 %end; space
	    %if &i=1 %then %do; ("!S={borderbottomwidth=1}Unblinded Treatment Period" ucol1 ucol2 ucol3) %end; ;  
		...
		define formal   / style=[cellwidth=20% just=left] "System Organ Class~ Preferred Term" flow;  
	  	...
	  	%end;  
			...
			%if &i=2 %then %do;
  		    define col99     / style=[cellwidth=8% just=left] "Blinded Overall~ (N=&col99)";
			define ucol9     / style=[cellwidth=7% just=left] "Unblinded Overall~ (N=&col92)";
			%end;
	  	%if &i=1 %then %do;
...
data openses.&pgmqc; 
   	retain formal stat col99  ucol9;   
   	set final; 
   	keep formal stat col99 ucol9;  
	%fmtqc;
run; 
